...
import { setThreadVoteOptimistic, setCommentVoteOptimistic } from "@/redux/slice/votes";
...
export default function ThreadDetailPage() {
  ...
  const { threadDetail, threadDetailStatus } = useSelector((state) => state.threads);
  ...
  const handleThreadUpvote = () => {
    ...
    if (isUpvoted) {
      dispatch(setThreadVoteOptimistic({ threadId: threadDetail.id, voteType: 0, userId: profile?.id }));
      dispatch(neutralizeThreadVote(threadDetail.id));
    } else {
      dispatch(setThreadVoteOptimistic({ threadId: threadDetail.id, voteType: 1, userId: profile?.id }));
  ...
  const handleCommentUpvote = (comment) => {
    ...
    if (isUpvoted) {
      dispatch(setCommentVoteOptimistic({ commentId: comment.id, voteType: 0, userId: profile?.id }));
      dispatch(neutralizeCommentVote({ threadId: threadDetail.id, commentId: comment.id }));
    } else {
      dispatch(setCommentVoteOptimistic({ commentId: comment.id, voteType: 1, userId: profile?.id }));
      dispatch(upvoteComment({ threadId: threadDetail.id, commentId: comment.id }));
    }
  };
  ...
  const handleCommentDownvote = (comment) => {
    ...
    if (isDownvoted) {
      ...
      dispatch(setCommentVoteOptimistic({ commentId: comment.id, voteType: -1, userId: profile?.id }));
      dispatch(downvoteComment({ threadId: threadDetail.id, commentId: comment.id }));
    }
  };
  ...
  const owner = threadDetail?.owner || (threadDetail?.ownerId && usersMap[threadDetail.ownerId]);
        ...
        <div className="flex flex-col max-w-xl mx-auto sm:border sm:border-gray-300 lg:shadow rounded-2xl p-4">
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-8 text-gray-800 py-4 border-t border-b border-gray-200">
              <button
                className={`flex gap-1 items-center ${threadDetail.upVotesBy?.includes(profile?.id) ? 'text-zinc-900 font-bold' : ''}`}
              ...
              <button
                className={`flex gap-1 items-center ${threadDetail.downVotesBy?.includes(profile?.id) ? 'text-zinc-900 font-bold' : ''}`}
              ...
              <div className="flex gap-1 items-center">
                <ChatBubbleOvalLeftIcon size={20} /> {threadDetail.comments?.length || 0}
              </div>
            </div>
          </div>
              ...
              <form onSubmit={handleCommentSubmit}>
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    disabled={createCommentStatus.isLoading}
                  >
                    {createCommentStatus.isLoading ? "Submitting..." : "Submit Comment"}
                  </button>
                </div>
              </form>
            )}
                ...
                {threadDetail.comments.map((comment) => (
                  <div key={comment.id} className="border-b border-gray-200 pb-4">
                    ...
                    <div className="flex gap-4 mt-2 pl-8 text-sm">
                      <button
                        className={`flex gap-1 items-center ${comment.upVotesBy?.includes(profile?.id) ? 'text-zinc-900 font-bold' : ''}`}
                      ...
                      <button
                        className={`flex gap-1 items-center ${comment.downVotesBy?.includes(profile?.id) ? 'text-zinc-900 font-bold' : ''}`}
